...
function writeCredentialsDoc({
  ...
  const lines = [
    ...
    '## Staff logins',
    '',
    '**All staff** (admin, picker, driver) sign in at **`/admin-login`** only. After login the app routes by role (`/dashboard`, `/picker` queue, or `/driver` jobs). Do not use `/picker` or `/driver` as login URLs.',
    '',
    '| Role | Email | Password | Lands on (after `/admin-login`) |',
    '|------|-------|----------|--------------------------------|',
