// scripts/generate-general-purpose-location-images.mjs
// One-shot generator for the 50 generic-purpose (place-agnostic) cinematic
// golden-hour location hero images. Sister to generate-location-images.mjs.
//
// Reads scripts/location-general-purpose-prompts.json, calls the
// matrix_generate_image MCP in batches of 5, downloads each result, and
// converts to 1920w WebP at quality 80 in public/locations/<slug>.webp —
// matching the size/quality of the existing hero assets and the
// place-specific location pool, so the 21:9 hero strip renders the same
// regardless of which pool the page falls back to.
//
...
//
// Usage:
//   node scripts/generate-general-purpose-location-images.mjs                  # full run
//   node scripts/generate-general-purpose-location-images.mjs --only english-thatched-cottage-honey
//   node scripts/generate-general-purpose-location-images.mjs --batch 5         # only 5 images
//   node scripts/generate-general-purpose-location-images.mjs --from 20         # starting at index 20
//
// Why this file is separate from generate-location-images.mjs:
//   The original is wired to scripts/location-images-prompts.json — the 60
//   hand-picked place-specific images (Guildford, Windsor, Whitstable, etc.).
//   This file mirrors the same pattern so we can extend the pool with 50
//   generic-purpose images without touching the place-specific pipeline.
//   Both feeds land in public/locations/ and are registered together in
...
const PROMPTS_PATH = path.join(ROOT, 'scripts', 'location-general-purpose-prompts.json')
const PUBLIC_LOCATIONS_DIR = path.join(ROOT, 'public', 'locations')
const TMP_DIR = path.join(ROOT, '.tmp', 'location-general-purpose-images')

const TARGET_WIDTH = 1920
const QUALITY = 80
const BATCH_SIZE = 5 // Smaller batches dodge intermittent 5-image rate limits
const BATCH_DELAY_MS = 3000

function parseArgs() {
  const args = process.argv.slice(2)
  const opts = { only: null, batch: null, from: null }
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--only' && args[i + 1]) {
      ...
      opts.from = parseInt(args[i + 1], 10)
      i++
    }
  }
  return opts
...
async function main() {
  ...
  if (opts.from !== null) {
    toProcess = toProcess.slice(opts.from)
  }

  console.log(
    `Generating ${toProcess.length} generic-purpose image${toProcess.length === 1 ? '' : 's'} in batches of ${BATCH_SIZE}...\n`,
  )
  ...
  for (let i = 0; i < toProcess.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      // Persist remaining items to a checkpoint so we can resume after quota
      // refresh without re-running the API calls we've already paid for.
      const remaining = toProcess.slice(i + BATCH_SIZE)
      if (remaining.length > 0) {
        const checkpoint = path.join(TMP_DIR, 'checkpoint.json')
        await fs.writeFile(checkpoint, JSON.stringify(remaining, null, 2), 'utf-8')
        console.log(`  -> Wrote ${remaining.length} remaining slug${remaining.length === 1 ? '' : 's'} to ${checkpoint}`)
      }
      break
    }
